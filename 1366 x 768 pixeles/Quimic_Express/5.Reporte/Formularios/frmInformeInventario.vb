...
Public Class FrmInformeInventario
    ...
    Public Sub cargarInforme(condicion As String)
        ...
        Dim sql As String
        If condicion = "" Then
            sql = "select * from consultainventario"
        Else
            sql = "select * from consultainventario where " & condicion
        End If
        Try
            abrirConexion()
            adaptador = New MySqlDataAdapter(sql, conexion)
            datos = New DataSet
            adaptador.Fill(datos, "xInventario")
        ...
        Catch ex As MySqlException
            MsgBox(ex.Message)
        End Try
        rptContenedor.LocalReport.ReportEmbeddedResource = "Quimic_Express.rptProductos.rdlc"
        Dim rptDataSet As New ReportDataSource("cd_inventario", datos.Tables("xInventario"))
