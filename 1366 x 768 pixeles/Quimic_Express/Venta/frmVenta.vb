...
Public Class frmVenta
    ...
    Private Sub agregarItems()
        ...
        While c < cant
            Try
                sql = "insert into venta_items (idventa,idproducto,descripcion,precio,cantidad,iva,estadodescuento) values(@idventa,@idproducto,@descripcion,@precio,@cantidad,@iva,@estado,@descuento)"
                abrirConexion()
    ...
    Private Sub ActualizarVenta()
        ...
        sql = "update venta set comprobante= @comp, idcliente=@idcli, fecha=@fecha, subtotal=@subt,iva=@iva, descuento=@desc, totalneto=@totalneto, idusuario=@iduser, estado=@estado, ruc=@ruc where id=@id"
        Try
            ...
            While c < cant
                ...
                sql = "insert into venta_items (idventa,idproducto,descripcion,precio,cantidad,iva,estado,ruc,descuento,subtotal) values(@idventa,@idproducto,@descripcion,@precio,@cantidad,@iva,@estado,@ruc,@descuento,@subtotal)"
                comando = New MySqlCommand(sql, conexion)
                comando.Parameters.AddWithValue("@idventa", txtidVenta.Text)
                comando.Parameters.AddWithValue("@idproducto", grilla.Item(1, c).Value)
                comando.Parameters.AddWithValue("@descripcion", grilla.Item(2, c).Value)
    ...
    Public Sub verificarVenta()
        ...
        Dim adaptador As MySqlDataAdapter
        usuario = 1
        sql = "select * from venta where idusuario='" & usuario & "' and estado=0"
        ...
        Try
            ...
            Else
                ...
                sql = "select max(cast(comprobante as unsigned)) as comprobante from venta"
                adaptador = New MySqlDataAdapter(sql, conexion)
                ...
                sql = "INSERT INTO venta(comprobante,idcliente,fecha,subtotal,iva,descuento,totalneto,idusuario,estado,ruc) values(@comprobante,@idcliente,@fecha,@subtotal,@iva,@descuento,@totalneto,@idusuario,@estado,ruc)"
                Dim comando As MySqlCommand
                comando = New MySqlCommand(sql, conexion)
                ...
                sql = "select * from venta where idusuario='" & usuario & "' and estado=0 order by id desc limit 1"
    ...
    Private Sub F6BuscarClienteToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles F6BuscarClienteToolStripMenuItem.Click
        Dim frm As frmBuscarCliente = New frmBuscarCliente
        AddOwnedForm(frm)
        frm.Show()
        frm.frmOrigen = "venta"
    End Sub
    ...
    Private Sub buscarCliente()
        ...
        Try
            ...
            Else
                ...
                Dim frmx As frmBuscarCliente = New frmBuscarCliente
                AddOwnedForm(frmx)
                frmx.Show()
    ...
    Private Sub btnGuardar_Click(sender As Object, e As EventArgs) Handles btnGuardar.Click
        ...
        frmPrintVenta.Show()
        frmPrintVenta.imprimirVenta(txtidVenta.Text)
    ...
    Private Sub btnCredito_Click(sender As Object, e As EventArgs) Handles btnCredito.Click
        AbrirFormInPanel(New frmVentaCredito)
    End Sub
    ...
    Private Sub txtIdProducto_KeyPress(sender As Object, e As KeyPressEventArgs) Handles txtIdProducto.KeyPress
        If e.KeyChar = ChrW(Keys.Enter) Then
            If txtIdProducto.Text <> "" Then
                ...
                e.Handled = True
            Else
                Dim frmx As frmBuscarProducto = New frmBuscarProducto
                AddOwnedForm(frmx)
                frmx.Show()
                frmx.frmOrigen = "venta"
            End If
